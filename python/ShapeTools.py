...
class ShapeBuilder(ModelBuilder):
        ...
        for i, b in enumerate(self.DC.bins):
                ...
                if extranorm:
                    if self.options.packAsymPows:
                        if coeff.ClassName() == "ProcessNormalization":
                            ...
                            coeff = self.addObj(
                                ROOT.ProcessNormalization,
                                "n_exp_final_bin%s_proc_%s" % (b, p),
                                "",
                                coeff.getVal(),
                            )
                        else:
            ...
            if channelBinParFlag:
                ...
                sum_s = self.addObj(
                    ...
                    "pdf_bin%s" % b,
                    "",
                    ROOT.RooArgList(prop),
                    ROOT.RooArgList(self.out.var("ONE")),
                    True,
                )
            ...
            addSyst = False
            if self.options.moreOptimizeSimPdf == "none":
                addSyst = True
            ...
            # always be added.
            elif self.options.moreOptimizeSimPdf == "lhchcg":
                addSyst = True
            elif self.options.moreOptimizeSimPdf == "cms":
                if self.options.noOptimizePdf:
                    raise RuntimeError("--optimize-simpdf-constraints=cms is incompatible with --no-optimize-pdfs")
                addSyst = False
            ...
            if channelBinParFlag and not self.options.noHistFuncWrappers:
                for idx in range(pdfs.getSize()):
                    ...
                    wrapper.setStringAttribute(
                        "combine.process",
                        pdfs.at(idx).getStringAttribute("combine.process"),
                    )
                    wrapper.setStringAttribute(
                        "combine.channel",
    ...
    def prepareAllShapes(self):
        ...
        if shapeTypes.count("TH1") == len(shapeTypes):
            ...
            shapeTypes.count("RooDataSet") > 0 or shapeTypes.count("TTree") > 0 or len(list(shapeObs.keys())) > 1
    ...
    def getShape(self, channel, process, syst="", _cache={}, allowNoSyst=False):
        ...
        if channel in self.DC.shapeMap:
            ...
            raise KeyError("Shape map has no entry for channel '%s'" % (channel))
        names = []
        ...
        if len(names) == 1 and names[0] == "FAKE":
            return None
        if syst != "":
            if len(names) == 2:
                if allowNoSyst:
        ...
        for mp in self.options.modelparams:
            if len(mp.split("=")) != 2:
                raise RuntimeError("No value found for keyword in %s (use --keyword-value WORD=VALUE)" % mp)
            mpname, mpv = mp.split("=")
            protected_kwords = ["PROCESS", "CHANNEL", "SYSTEMATIC", "MASS"]
            if mpname in protected_kwords:
                raise RuntimeError("Cannot use the following keywords (already assigned in combine): $" + " $".join(protected_kwords))
        ...
        if ":" in objname and not isinstance(file, DataFrameWrapper):  # workspace:obj or ttree:xvar or th1::xvar
            ...
            if self.wsp.ClassName() == "RooWorkspace":
                ...
                rds = ROOT.RooDataSet(
                    "shape%s_%s_%s%s" % (postFix, process, channel, "_" + syst if syst else ""),
                    "shape%s_%s_%s%s" % (postFix, process, channel, "_" + syst if syst else ""),
                    self.wsp,
                    ROOT.RooArgSet(self.out.var(oname)),
                    "",
                    "__WEIGHT__",
                )
                rds.var = oname
                ...
                for X in _neverDelete:
                    if X.InheritsFrom("TNamed") and X.GetName() == name:
                        return X
                self.doVar(
                    "%s[%f,%f]"
                    % (
                        oname,
                        self.wsp.GetXaxis().GetXmin(),
                        self.wsp.GetXaxis().GetXmax(),
                    )
                )
    ...
    def getPdf(self, channel, process, _cache={}):
        ...
        for syst, nofloat, pdf, args, errline in self.DC.systs:
            ...
            if errline[channel][process] != 0:
                if allowNoSyst and not self.isShapeSystematic(channel, process, syst):
                    continue
                systShapeName = syst
                ...
                if shapeUp.ClassName() != shapeNominal.ClassName() and nominalPdf.ClassName() != "RooParametricHist":
                    raise RuntimeError("Mismatched shape types for channel %s, process %s, syst %s" % (channel, process, syst))
        ...
        for syst, scale, pdfUp, pdfDown in morphs:
            ...
            if scale == 1:
                ...
                coeffs.add(
                    self.doObj(
                        "%s_scaled_%s_%s" % (syst, channel, process),
                        "prod",
                        "%s, %s" % (scale, syst),
                    )
                )
        ...
        if "2a" in shapeAlgo:  # old shape2
            ...
            _cache[(channel, process)] = ROOT.VerticalInterpPdf(
                "shape%s_%s_%s_morph" % (postFix, channel, process),
                "",
                pdfs,
                coeffs,
    ...
    def getExtraNorm(self, channel, process):
        ...
        for syst, nofloat, pdf, args, errline in self.DC.systs:
            if "shape" not in pdf:
                continue
            if errline[channel][process] != 0:
                if pdf[-1] == "?" and not self.isShapeSystematic(channel, process, syst):
                    continue
                systShapeName = syst
                if (syst, channel, process) in list(self.DC.systematicsShapeMap.keys()):
                    systShapeName = self.DC.systematicsShapeMap[(syst, channel, process)]
                shapeUp = self.getShape(channel, process, systShapeName + "Up")
                shapeDown = self.getShape(channel, process, systShapeName + "Down")
                if shapeUp.ClassName() != shapeNominal.ClassName() and shapeNominal.ClassName() != "RooParametricHist":
                    raise RuntimeError("Mismatched shape types for channel %s, process %s, syst %s" % (channel, process, syst))
                if shapeDown.ClassName() != shapeNominal.ClassName() and shapeNominal.ClassName() != "RooParametricHist":
                    raise RuntimeError("Mismatched shape types for channel %s, process %s, syst %s" % (channel, process, syst))
                ...
                if self.options.packAsymPows:
                    ...
                    obj_kappaDown = self.addObj(
                        ROOT.RooConstVar,
                        "%f" % kappasScaled[0],
                        "",
                        float("%f" % kappasScaled[0]),
                    )
                    obj_kappaUp = self.addObj(
                        ROOT.RooConstVar,
                        "%f" % kappasScaled[1],
                        "",
                        float("%f" % kappasScaled[1]),
                    )
                    obj_var = self.out.var(syst)
                    self.addObj(
                        ROOT.AsymPow,
                        "systeff_%s_%s_%s" % (channel, process, syst),
                        "",
                        obj_kappaDown,
                        obj_kappaUp,
                        obj_var,
        ...
        if shape == None:
            ...
            if name not in _cache:
                ...
                if self.out.mode == "binned":
                    ...
                    _cache[name] = rds
            return _cache[name]
        if shape.GetName() not in _cache:
    ...
    def shape2Pdf(self, shape, channel, process, _cache={}):
        ...
        if shape == None:
            ...
            return _cache[name]
        if shape.GetName() + "Pdf" not in _cache:
            if shape.ClassName().startswith("TH1"):
                if self.options.useHistPdf == "never":
                    ...
                    rhp = ROOT.RooHistPdf(
                        "%sPdf" % shape.GetName(),
                        "",
                        ROOT.RooArgSet(self.out.var(self.TH1Observables[channel])),
                        rdh,
                    )
    ...
    def checkRooAddPdf(self, channel, process, pdf):
        ...
        if coeflist.getSize() == pdf.pdfList().getSize() - 1:
            return True
        sum = 0.0
        for i in range(coeflist.getSize()):
        ...
        if pdf.ClassName() == "FastVerticalInterpHistPdf2D":
            ...
            if f2.ClassName() == "FastVerticalInterpHistPdf2D" and f2.conditional():
                ...
                ret = ROOT.RooProdPdf(
                    ...
                    "",
                    ROOT.RooArgSet(f1),
                    ROOT.RooFit.Conditional(ROOT.RooArgSet(f2), ROOT.RooArgSet(f2.y())),
                )
                ret.optf2 = f2
                ret.optf1 = f1
